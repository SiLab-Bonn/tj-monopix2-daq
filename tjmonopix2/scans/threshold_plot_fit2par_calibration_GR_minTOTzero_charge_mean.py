...
#     latest_file = find_latest_file(folder_path, '_threshold_scan_interpreted.h5')
#     if latest_file is None:
#         raise FileNotFoundError(f"Nessun file .h5 trovato in {folder_path}")
#         sys.exit(0)
#     # Prendi solo il nome del file, senza il path
#     file_name = os.path.basename(latest_file)
#     print(f"Nessun file specificato: uso il più recente {file_name} in {folder_path}")
...
def clean_data_debug(results):
    ...
    with PdfPages('tot_distribution_charge_dac_ALL.pdf') as pdf:
        for i, row in enumerate(results): # i is the charge in DAC
            plt.figure()
            ...
            try:
                ...
                #print(f'i N std1 mean_err std1/sqrt(N) 1/sqrt(12) {i:.0f}, {N:.0f}, {std1:.5f}, {mean_err:.5f}, {std1/np.sqrt(N):.5f}, {1/np.sqrt(12):.5f}')
                #print(f'i={i:.0f}, N={N:0f}, std1={std1:.2e}, mean_err= {mean_err:.2e}, std1/sqrt(N)={std1/np.sqrt(N):.2e}, 1/sqrt(12)={1/np.sqrt(12):.2e}')
                #std = 1/np.sqrt(12)
                #std = std1/np.sqrt(N)
                #std = mean_err
                #std = std1/20. # cheating reduced the error to get chi2=dof=97
                std = std1
                # sigma_charge = 3.3 # noise in quadrature with thr dispersion
                # fit3p_ini = (8.06, 74.81, 26.74) # col 290
                # std = derivate_func(i, *fit3p_ini)*sigma_charge
                # print(f'i={i:.0f}, sigma_effective={std:.2e}')
...
def clean_data_by_charge(results):
    """
    Fit a Gaussian to each col (for each ToT bin take the mean of the distribution of the Qinj) of the results array and get the mean and std.

    Args:
    results: 2D numpy array of shape (n, m) where n is the number of rows (Qinj values) and m is the number of columns (TOT values) and contains N hits with that TOT
    x: 1D numpy array of shape (m,) representing the x-axis values.
    ...
    means, stds, rows = [], [], []
    # Iterate over columns (ToT) and fit with gauss function the Qinj distribution for that ToT
    with PdfPages('charge_DAC_tot_distribution_ALL.pdf') as pdf:
        for i, row in enumerate(results.T):
            plt.figure()
            plt.plot(np.arange(len(row)), row)
            x = np.arange(len(row))
            #print('len di row',len(row))
            plt.title(f"Charge distribution for TOT {i}")
            ...
            # se N < Nmin --> set mean = 0 e skip append
            if N < Nmin or i > 120:
                mean = 0
                std = 0
                if N !=0:
                    print(f"skip ToT={i}, N={N} < Nmin={Nmin}, set mean={mean}, std={std}")
                continue   # skip and go to next ToT
            ...
            try:
                ...
                perr = np.sqrt(np.diag(pcov))  # errori standard sui parametri
                A_err, mean_err, std_err = perr
                #print(f'mean_charge={mean:.2f}, std1={std1:.2f}, mean_err={mean_err:.2e},  std1/sqrt(N)={std1/np.sqrt(N):.2e}, N={N:.0f}')
                #std = std1/np.sqrt(N)
                #print('mean , error on mean used=std1/sqrt(N)',mean, std)
                #std = mean_err
                #print('mean , error on mean used from fit',mean, std)
                std = std1
                #print('mean , error on mean used=std',mean, std)
                # print(f'mean_err={mean_err:.2e},  std1/sqrt(N)={std1/np.sqrt(N):.2e}')

                # plot del fit
                x_fit = np.linspace(0, len(row)-1, 500)
                plt.plot(x_fit, gauss(x_fit, *popt), 'r--', label=f'Gaussian fit, N={N:.0f}')
                func_params_gauss = ['ampl', 'mean', 'std']
                for j, param in enumerate(popt):
                        #plt.text(10, 35 - j * 5, f'{func_params_gauss[j]}={param:.2f} ± {np.sqrt(pcov[j, j]):.2f}', color='k', fontsize=14)
                        plt.annotate(f'{func_params_gauss[j]} = {param:.2f} ± {np.sqrt(pcov[j,j]):.2f}',
                                   xy=(0.05, 0.95 - j*0.05), xycoords='axes fraction',fontsize=12, color='k', ha='left', va='top')
...
def inv_func_d_fixed(tot, a, b, d=d_fixed): # from https://github.com/SiLab-Bonn/tj-monopix2-daq/blob/development/tjmonopix2/analysis/analysis_utils.py
    #d = 27.
    print('inv_func_d_fixed, d_fixed',d_fixed)
    print('d',d)
    return (np.sqrt(b**2 * (a - tot)**2 + 2 * b * d * (a + tot) + d**2) - b * a + b * tot + d) * 0.5
    # return (np.sqrt(b**2 * (a - (tot-1.))**2 + 2 * b * d * (a + (tot-1.)) + d**2) - b * a + b * (tot-1.) + d) * 0.5
...
def chisq(f, x, y, dy, pars, label="Chi²"):
    ...
    #ndof = len(y) - len(pars)
    ndof = len(y) - (len(pars) - 1) # 1 param is fixed in the func_3par
...
if __name__ == '__main__':
    ...
    # Create a PDF file
    with pdf.PdfPages(os.path.join(folder_path, base_name + '_tot_calibration_fit2par_charge_mean.pdf')) as pdf_file:
        # Plot 1: S-Curve Plot
        print(' ')
        print('plot s-curves')
        region1 = [0,224, start_row, stop_row]
        ...
        # Plot the image
        image = ax.imshow(s_curve_x0[:, :], vmin=26, vmax=29)
        ...
        for r,region in enumerate(regions):
            ...
            # # Plot the image
            # #image = ax.imshow(s_curve_x0[:, :], vmin=26, vmax=29)
            # min_thr_pl = np.round(popt[1]-2.5*popt[2])
            # max_thr_pl = np.round(popt[1]+2.5*popt[2])
            # image = ax.imshow(s_curve_x0[:, :], vmin=min_thr_pl, vmax=max_thr_pl)

            # # Set the color bar limits
            # #cbar = fig.colorbar(image, ax=ax, ticks=np.arange(0, 501, 100))
            # cbar = fig.colorbar(image, ax=ax, ticks=np.arange(min_thr_pl, max_thr_pl, 1))
            # cbar.ax.set_yticklabels(np.arange(min_thr_pl, max_thr_pl, 1), fontsize=12)
            # cbar.set_label('Threshold [DAC]', fontsize=14)
            # #cbar.ax.set_yticklabels(np.arange(0, 501, 100), fontsize=12)
       ...
       # Plot 5: Noise distribution
        ...
        for i, tot_cal in enumerate(tot_cal_array):
            ...
            try:
                ...
                tot_int = rows
                tot_err = np.full_like(tot_int, 1/np.sqrt(12), dtype=float)
                charge_mean = means
                charge_err = stds
                print('tot_int')
                print([f'{m:.2f}' for m in tot_int])
                print('tot_err')
                print([f'{m:.2f}' for m in tot_err])
                print('charge_mean')
                print([f'{m:.2f}' for m in charge_mean])
                print('charge_err')
                ...
                # add also the points used as input to the fit (charge mean for each ToT) on the ToT map
                plt.errorbar(charge_mean, tot_int, yerr=tot_err, fmt='ro', label='mean charge', markersize=3)
                #plt.errorbar(charge_mean, tot_int, tot_err, charge_err, 'ro', label='mean charge', markersize=3)
                ...
                try:
                    ...
                    # fit using the func function and error on y=tot variable only
                    #popt_cal, pcov_cal = curve_fit(func, charge_mean, tot_int,
                    #                             sigma=tot_err, p0=p0, bounds=param_bounds)
                    # fit using the inv_func function and error on y=charge variable only
                    #popt_cal, pcov_cal = curve_fit(inv_func, tot_int, charge_mean,
                    #                             sigma=charge_err, p0=p0, bounds=param_bounds)

                    # fit with 2 param
                    popt_cal, pcov_cal = curve_fit(lambda x, a, b: func(x, a, b, d_fixed), charge_mean, tot_int,
                                                 sigma=tot_err, p0=p0, bounds=param_bounds)
                    # popt_cal, pcov_cal = curve_fit(lambda x, a, b: func_d_fixed(x, a, b), x_fit, means_fit,
                    #                                 sigma=stds_fit, p0=p0, bounds=param_bounds)  # don't use it
                    #popt_cal, pcov_cal = curve_fit(func_d_fixed, x_fit, means_fit,
                    #                             sigma=stds_fit, p0=p0, bounds=param_bounds)

                    # # dati sperimentali: x = charge_mean, y = tot_int
                    # data_odr = odr.RealData(charge_mean, tot_int, sx=charge_err, sy=tot_err)

                    # # modello
                    # model = odr.Model(func)

                    # # fit con stime iniziali p0
                    # odr_fit = odr.ODR(data_odr, model, beta0=p0)
                    # odr_fit.set_job(fit_type=0)   # 0 = regressione ortogonale

                    # # esegui fit
                    # output = odr_fit.run()

                    # # parametri ottimizzati e incertezze
                    # popt_cal = output.beta          # parametri stimati
                    # perr_cal = output.sd_beta       # errori standard
                    # #cov_beta = output.cov_beta      # matrice di covarianza stimata
                    # pcov_cal = output.cov_beta      # matrice di covarianza stimata
                    ...
                    # def 3 par to use the func with 3 par
                    params = list(popt_cal) + [d_fixed]  # [a, b, d_fixed]
                    params = np.array(params)

                    # Calcolo del chi²
                    #chi2_val, ndof = chisq(func, x_fit, means_fit, stds_fit, popt_cal, label=f"Chi² - {section_name}")
                    #chi2_val, ndof = chisq(func, charge_mean, tot_int, tot_err, popt_cal, label=f"Chi² - {section_name}")
                    #chi2_val, ndof = chisq(inv_func, tot_int, charge_mean, charge_err, popt_cal, label=f"Chi² - {section_name}")

                    chi2_val, ndof = chisq(func, charge_mean, tot_int, tot_err, params, label=f"Chi² - {section_name}")
                    #chi2_val, ndof = chisq(func, x_fit, means_fit, stds_fit, params, label=f"Chi² - {section_name}")
                    ...
                    plt.text(10, 40, '$f_{2par}(x)=(a/x +1/b)*(x-THR)$', color='k', fontsize=14)
                    ...
                    # Print fit results
                    print('Fit results with f_2par')
                    func_params = ['a', 'b', 'THR']
                    #func_param_values = popt_cal
                    func_param_values = list(popt_cal) + [d_fixed]
                    formatted = ", ".join([f"{v:.2f}" for v in func_param_values])
                    print("Fit results with f_2par:", formatted)
                    ...
                    plt.text(10, 35 - 2 * 5, f'{func_params[2]}={d_fixed:.2f}', color='k', fontsize=14)
                    print(f'{func_params[2]}={d_fixed:.2f}')
                    ...
                    if len(x_plot) == 0:
                        ...
                        ax1.errorbar(charge_mean, tot_int, yerr=abs(tot_err), fmt='o', label='Fit Data', color='blue', markersize=6)

                        # Curva fittata
                        ax1.plot(x_plot, func(x_plot, *params), 'k--', label='Fit: $f(x)$')

                        #ax1.plot(inv_func(means, *popt_cal), means, 'ro', label='inverted func: $inv_f(meansToT)$')

                        # add points with the TOT integer from 0 t0 20 and the associated charge calculated with the inv_func
                        tot_test = np.arange(0, 21)
                        ax1.plot(inv_func(tot_test, *params), tot_test, 'go', label='inverted func: $inv_f(int ToT)$')
                        ...
                        res = tot_int - func(charge_mean, *params)
                        #res = means_fit - func(x_fit, *popt_cal)
                        #res = means - func(x, *popt_cal)

                        ax2.errorbar(charge_mean, res, yerr=abs(tot_err),color='blue', fmt='o')
                ...
                if i ==0:
                    ...
                    popt_cal_NF_CASC = list(popt_cal) + [d_fixed]
                    chi2_ndof_cal_NF_CASC = chi2_ndof
                elif i ==2:
                    popt_cal_HV_CASC = list(popt_cal) + [d_fixed]
                    chi2_ndof_cal_HV_CASC = chi2_ndof
                elif i ==3:
                    popt_cal_HV = list(popt_cal) + [d_fixed]
        ...
        #        folder_path, f'{folder_name} {"NF_casc.png" if i == 1 else "NF.png"}'), format='png')
        #    plt.savefig(os.path.join(
        #        folder_path, f'{folder_name} {"NF_casc.pdf" if i == 1 else "NF.pdf"}'), format='pdf')
        #    pdf_file.savefig()  # Add this plot to the PDF file
        #print('THIS IS FINE')
        ...
        #             width = 5
        #             #print(max_value, center)
        #             print('thr initial', max_value, center, width)
        #             p0 = [max_value, center, width]
        #             popt, pcov = [0,0,0,0],0
        #             popt, pcov = curve_fit(gauss, x_fit, hist[1:], p0=p0)
        #             plt.plot(x_fit, gauss(x_fit, *popt), 'r-', label=label+ ' fit')
        #             mean, std = popt[1], popt[2]
        #             print('fitted thr and std',mean,std)
    ...
    new_file_name = base_name + '_tot_calibration_fit2par_charge_mean.h5'
    ...
    print(f"[INFO] File selezionato: {file_name}")
    #print("Fit results with f_3par:", formatted)
    print(f"THR average: {thresholds[1,0]:.2f}")
    print("thresholds all regions", [[f"{thr:.1f}", f"{std:.1f}"] for thr, std in thresholds])
