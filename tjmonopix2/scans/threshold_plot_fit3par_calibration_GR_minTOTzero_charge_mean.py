...
def clean_data_by_charge(results):
    ...
    with PdfPages('charge_DAC_tot_distribution_ALL.pdf') as pdf:
        for i, row in enumerate(results.T):
            ...
            # print('len di x',len(x))
            # mean_guess = np.average(x, weights=row)       # media pesata
            # std_guess = np.sqrt(np.average((x - mean_guess)**2, weights=row))  # varianza pesata

            # p0 = [row.max(), mean_guess, std_guess]
            # print ('i=TOT, p0,  N, Nmin', i, p0, N, Nmin)
...
if __name__ == '__main__':
    ...
    # Create a PDF file
    with pdf.PdfPages(os.path.join(folder_path, base_name + '_tot_calibration_fit3par_charge_mean.pdf')) as pdf_file:
        # Plot 1: S-Curve Plot
        print(' ')
        print('plot s-curves')
        region1 = [0,224, start_row, stop_row]
        ...
        for i, tot_cal in enumerate(tot_cal_array):
            ...
            try:
                ...
                try:
                    ...
                    # # add to y errors also propagation of x errors
                    # print ('charge_err')
                    # print([f'{m:.2f}' for m in charge_err])
                    # print ('tot_err')
                    # print([f'{m:.2f}' for m in tot_err])
                    # tot_err_add = np.array([derivate_func(val, *popt_cal) for val in charge_mean]) * charge_err
                    # tot_err_new = np.sqrt(tot_err**2+tot_err_add**2)
                    # print("tot_err_add")
                    # print([f'{m:.2f}' for m in tot_err_add])
                    # print("tot_err_new")
                    # print([f'{m:.2f}' for m in tot_err_new])

                    # tot_err = np.sqrt(tot_err**2+tot_err_add**2)
                    # print ('updated tot_err')
                    # print([f'{m:.2f}' for m in tot_err])

                    # popt_cal, pcov_cal = curve_fit(func, charge_mean, tot_int,
                    #                              sigma=tot_err, p0=p0, bounds=param_bounds)

                    # # add also the points used as input to the fit (charge mean for each ToT) on the ToT map with charge error used
                    # plt.errorbar(charge_mean, tot_int, xerr=charge_err, fmt='ro', label='mean charge, charge_err', markersize=3)
                    ##plt.errorbar(charge_mean, tot_int, yerr=tot_err, fmt='ro', label='mean charge', markersize=3)
                    ...
                    plt.text(100, 35 - 5, f'cost=$b_{{4par}}$=a-d/b={popt_cal[0]-popt_cal[2]/popt_cal[1]:.2f}', color='k', fontsize=14)
                    ...
                    if len(x_plot) == 0:
                        ...
                        res = tot_int - func(charge_mean, *popt_cal)
    ...
    new_file_name = base_name + '_tot_calibration_fit3par_charge_mean.h5'
