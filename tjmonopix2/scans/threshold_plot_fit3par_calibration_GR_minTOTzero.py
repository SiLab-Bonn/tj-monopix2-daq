...
def clean_data_debug(results):
    """
    Fit a Gaussian to each row (each Qinj) of the results array and get the mean and std of the ToT for taht Qinj
    Fit a Gaussian to each row (each Qinj) of the results array and get the mean and std of the ToT for taht Qinj
    ...
    x = np.arange(0, 128)
    means, stds, rows = [], [], []
    # Iterate over rows (Qinj) and fit with gauss function the TOT distribution for each Qinj
        ...
        for i, row in enumerate(results): # i is the charge in DAC
            ...
            try:
                ...
                for j, param in enumerate(popt):
                        ...
                        plt.annotate(f'{func_params_gauss[j]} = {param:.4f} ± {np.sqrt(pcov[j,j]):.4f}',
