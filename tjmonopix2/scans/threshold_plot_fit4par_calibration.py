...
# Define the fit function (4 parameters)
...
if __name__ == '__main__':
    ...
    # Create a PDF file
    # with pdf.PdfPages(os.path.join(folder_path, file_name + '_calibration_4par.pdf')) as pdf_file:   #MODIFICA 1
    with pdf.PdfPages(os.path.join(folder_path, base_name + '_tot_calibration_fit4par.pdf')) as pdf_file:
        # plot 0: tot cal
        for i, tot_cal in enumerate(tot_cal_array):
            ...
            try:
                ...
                try:
                    ...
                    plt.text(10, 40, '$f_{4par}(x)=a\cdot x + b - (c/(x-t))$', color='k', fontsize=14)
                    ...
                    plt.text(0.05, 0.90, f"$\\chi^2$/ndof = {chi2_val:.2f}/{ndof} = {chi2_val/ndof:.3f}",
                    ...
                    if len(x_fit) == 0:
                        ...
                        ax2.plot(x_plot, np.full(x_plot.shape, 0.0), 'k--')
                        ax2.set_xlim(0, 250)
                        ax2.set_xlabel('d [m]')
    ...
    inj_tot_cal = np.zeros((512, 512, 5))
    ...
    inj_tot_cal[0:224, :, 0:4]   = popt_cal_NF
    inj_tot_cal[224:448, :, 0:4] = popt_cal_NF_CASC
    inj_tot_cal[448:480, :, 0:4] = popt_cal_HV_CASC
    inj_tot_cal[480:512, :, 0:4] = popt_cal_HV

    # Aggiungi chi2/ndof per pixel se hai mappe 2D
    inj_tot_cal[0:224, :, 4]     = chi2_ndof_cal_NF
    inj_tot_cal[224:448, :, 4]   = chi2_ndof_cal_NF_CASC
    inj_tot_cal[448:480, :, 4]   = chi2_ndof_cal_HV_CASC
    inj_tot_cal[480:512, :, 4]   = chi2_ndof_cal_HV
    ...
    new_file_name = base_name + '_tot_calibration_fit4par.h5'
    ...
    with tb.open_file(new_path, 'r+') as out_file:
        ...
        out_file.root.InjTotCalibration.attrs.columns = ['a', 'b', 'c', 't', 'chi2_ndof']
